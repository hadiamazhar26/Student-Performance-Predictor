...
Here are the evaluation results from my executed notebook:
...
* **Decision Tree Overfitting:** The single Decision Tree model shows classic signs of overfitting. It gets a very high $R^2$ of **0.966** on the training set, but drops to **0.703** on the test set. Averaging multiple trees in the Random Forest solves this issue, boosting test performance significantly.
* **Linear Regression Baseline:** The baseline Linear Regression model performs solidy ($R^2 = 0.724$), showing that many of the relationships in the data are close to linear.
...
* *Why it makes sense:* The student has excellent study habits, minimal absences, and a history of high scores, which aligns with a predicted high grade.
...
* *Why it makes sense:* A combination of low study time, high absences, historical failures, and very poor mid-term grades results in a failing grade prediction (below 10).
...
1. Run the Jupyter environment:
   ...
   jupyter notebook
   ```
2. Click on `notebook.ipynb` to view the code cells and pre-saved outputs. You can select **Cell -> Run All** from the top menu to re-run the entire pipeline.
